    ...
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 10 }}>
      {items.map(item => (
        <button key={item.id} onClick={() => toggle(item.id)} style={{
          ...
          color: isSelected(item.id) ? '#fff' : '#bbb', transition: 'all 0.15s',
        }}>
          {item.icon && <div style={{ fontSize: 22, marginBottom: 8 }}>{item.icon}</div>}
          <div style={{ fontSize: 14, fontWeight: 700 }}>{item.label}</div>
          {(item.sub || item.desc) && <div style={{ fontSize: 12, color: '#888', marginTop: 4, fontWeight: 500 }}>{item.sub || item.desc}</div>}
        </button>
      ))}
    </div>
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: values.length ? 10 : 0 }}>
        {values.map(tag => (
          <div key={tag} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(232,255,107,0.08)', border: '1px solid rgba(232,255,107,0.2)', borderRadius: 20, padding: '4px 12px', fontSize: 13, color: '#E8FF6B' }}>
            {tag}
            <button onClick={() => onChange(values.filter(v => v !== tag))} style={{ background: 'none', border: 'none', color: '#E8FF6B', cursor: 'pointer', fontSize: 15, lineHeight: 1, padding: 0 }}>×</button>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          placeholder={placeholder}
          style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px', color: '#fff', fontSize: 14, outline: 'none', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}
        />
        <button onClick={add} style={{ padding: '10px 16px', borderRadius: 10, background: 'rgba(232,255,107,0.1)', border: '1px solid rgba(232,255,107,0.2)', color: '#E8FF6B', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>Add</button>
      </div>
      <div style={{ fontSize: 12, color: '#888', marginTop: 6 }}>Press Enter or click Add after each one</div>
    </div>
  )
}
...
export default function OnboardingPage() {
  ...
  const recommendedAgents = [...new Set(
    data.goals.flatMap(g => Object.values(AGENT_ROLES).filter(r => r.goals.includes(g)).map(r => r.id))
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0B0F', color: '#fff', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ position: 'fixed', top: 3, left: 0, right: 0, height: 52, background: 'rgba(10,11,15,0.96)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', zIndex: 40 }}>
        <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: 2, color: '#E8FF6B' }}>◈ AGENTFORGE</div>
        <div style={{ display: 'flex', gap: 8 }}>
          {STEPS.map((s, i) => (
            <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ width: 28, height: 4, borderRadius: 4, background: i <= stepIndex ? '#E8FF6B' : 'rgba(255,255,255,0.08)', transition: 'background 0.3s' }} />
              <div style={{ fontSize: 10, color: i <= stepIndex ? '#E8FF6B' : '#555', letterSpacing: 0.5, fontWeight: 600 }}>{STEP_LABELS[s]}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize: 13, color: '#aaa', fontWeight: 600 }}>Step {stepIndex + 1} of {STEPS.length}</div>
      </div>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '96px 32px 80px' }}>
        <div className="fade-up">
          ...
          {step === 'identity' && (
            <>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(232,255,107,0.08)', border: '1px solid rgba(232,255,107,0.2)', borderRadius: 20, padding: '5px 14px', marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 12, color: '#E8FF6B', fontWeight: 700, letterSpacing: 1 }}>BUSINESS IDENTITY</span>
              </div>
              <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 10 }}>Let&apos;s build your business DNA</h2>
              <p style={{ color: '#bbb', fontSize: 16, marginBottom: 32, lineHeight: 1.7 }}>
                This is committed to memory across every agent. SAGE and EMBER will use your ethics and values to review every response.
              </p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div>
                  <label style={labelStyle}>Mission <span style={{ color: '#E8FF6B' }}>*</span></label>
                  <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Why does your business exist? What problem are you solving?</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Where are you going? What does success look like in 5–10 years?</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>What do you sell or offer? Be specific — your agents need to know this.</div>
                  <TextArea value={data.products} onChange={v => set('products', v)} placeholder="e.g. We sell ethically-made clothing including shirts, jackets, and accessories. We also offer a repair service and a buy-back scheme." rows={4} />
                </div>

                <div>
                  <label style={labelStyle}>Core Values <span style={{ color: '#E8FF6B' }}>*</span></label>
                  <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>What principles guide every decision you make?</div>
                  <TextArea value={data.valuesStatement} onChange={v => set('valuesStatement', v)} placeholder="e.g. Sustainability first. Radical transparency — we share our supply chain openly. Community over profit." rows={4} />
                </div>

                <div>
                  <label style={labelStyle}>Ethics Commitments</label>
                  <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>What lines will you never cross? SAGE will enforce these in every agent response.</div>
                  <TextArea value={data.ethicsStatement} onChange={v => set('ethicsStatement', v)} placeholder="e.g. We never use manipulative sales tactics or false scarcity. We never make claims we cannot back up with data." rows={4} />
                </div>

                <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24 }}>
                  <div style={{ fontSize: 12, color: '#E8FF6B', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 16 }}>Brand Voice</div>

                  <div style={{ marginBottom: 20 }}>
                    <label style={labelStyle}>Brand Voice Description</label>
                    <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>How does your brand sound? EMBER will use this to review every response.</div>
                    <TextArea value={data.brandVoice} onChange={v => set('brandVoice', v)} placeholder="e.g. We sound like a knowledgeable friend — warm, honest, and never condescending. Plain language, no jargon." />
                  </div>

                  <div style={{ marginBottom: 20 }}>
                    <label style={labelStyle}>Words & phrases to use</label>
                    <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Add words that feel on-brand.</div>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Add anything that feels off-brand or against your values.</div>
          ...
          {step === 'basics' && (
            <>
              <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 10 }}>Tell us about your business</h2>
              <p style={{ color: '#bbb', fontSize: 16, marginBottom: 32, lineHeight: 1.6 }}>Just the basics — we&apos;ll personalise everything from here.</p>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <ChoiceGrid items={BUSINESS_TYPES} selected={data.businessType} onSelect={(v: BusinessType) => set('businessType', v)} cols={4} />
              </div>
              <div>
                <label style={labelStyle}>Where are you at?</label>
                <ChoiceGrid items={BUSINESS_STAGES} selected={data.stage} onSelect={(v: BusinessStage) => set('stage', v)} cols={2} />
              </div>
            </>
          )}
          ...
          {step === 'goals' && (
            <>
              ...
              <p style={{ color: '#bbb', fontSize: 16, marginBottom: 32, lineHeight: 1.6 }}>Pick everything that applies — we&apos;ll build agents to match.</p>
              <ChoiceGrid items={GOALS} selected={data.goals} multi onSelect={(v: string[]) => set('goals', v)} cols={3} />
              {recommendedAgents.length > 0 && (
                <div style={{ marginTop: 20, padding: '16px 20px', background: 'rgba(232,255,107,0.05)', border: '1px solid rgba(232,255,107,0.15)', borderRadius: 12 }}>
                  <div style={{ fontSize: 13, color: '#E8FF6B', fontWeight: 700, marginBottom: 8 }}>
          ...
          {step === 'tone' && (
            <>
              <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 10 }}>How should your AI sound?</h2>
              <p style={{ color: '#bbb', fontSize: 16, marginBottom: 32, lineHeight: 1.6 }}>Your agents will match this tone in every interaction.</p>
                ...
                {TONES.map(t => (
                  <button key={t.id} onClick={() => set('tone', t.id)} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <span style={{ fontSize: 16, fontWeight: 800, color: data.tone === t.id ? '#fff' : '#ddd' }}>{t.label}</span>
                      <span style={{ fontSize: 13, color: '#aaa', fontWeight: 500 }}>{t.desc}</span>
                    </div>
                    <div style={{ fontSize: 13, color: '#888', fontStyle: 'italic' }}>&ldquo;{t.example}&rdquo;</div>
                  </button>
                ))}
              </div>
            </>
          ...
          {step === 'integrations' && (
            <>
              <h2 style={{ fontSize: 28, fontWeight: 800, marginBottom: 10 }}>Any tools to connect?</h2>
              <p style={{ color: '#bbb', fontSize: 16, marginBottom: 32, lineHeight: 1.6 }}>We&apos;ll link your agents to tools you already use. You can always add more later.</p>
              <ChoiceGrid items={INTEGRATIONS} selected={data.integrations} multi onSelect={(v: string[]) => set('integrations', v)} cols={4} />
              ...
              <div style={{ marginTop: 32, background: 'rgba(232,255,107,0.04)', border: '1px solid rgba(232,255,107,0.1)', borderRadius: 14, padding: '20px 24px' }}>
                <div style={{ fontSize: 12, color: '#E8FF6B', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 14 }}>Your Business Identity</div>
                {[
                  ...
                  <div key={row.label} style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 3 }}>{row.label}</div>
                    <div style={{ fontSize: 13, color: '#bbb', lineHeight: 1.6 }}>{row.value.length > 120 ? row.value.slice(0, 120) + '…' : row.value}</div>
                  </div>
                ) : null)}
                {data.brandKeywords.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ fontSize: 11, color: '#888', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>Brand Keywords</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                      {data.brandKeywords.map(k => (
                        <span key={k} style={{ fontSize: 12, background: 'rgba(232,255,107,0.08)', border: '1px solid rgba(232,255,107,0.15)', borderRadius: 10, padding: '2px 10px', color: '#E8FF6B' }}>{k}</span>
                      ))}
                    </div>
                  </div>
                )}
        ...
        {error && (
          <div style={{ margin: '20px 0', padding: '12px 16px', borderRadius: 10, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', color: '#F87171', fontSize: 14, fontWeight: 600 }}>{error}</div>
        )}

        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 40, paddingTop: 32, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <button onClick={() => stepIndex > 0 ? setStepIndex(i => i - 1) : router.push('/')} style={{ padding: '12px 24px', borderRadius: 12, background: 'none', border: '1px solid rgba(255,255,255,0.12)', color: '#aaa', cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>← Back</button>
          <button
            ...
            style={{
              ...
              border: 'none',
              color: canProceed() && !saving ? '#0A0B0F' : '#444',
...
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: 12, color: '#aaa',
